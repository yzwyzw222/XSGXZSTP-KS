package com.example.academic_entity_extract_kg_construction.infrastructure.integration;
...
public final class PortalAuthenticationFilter extends OncePerRequestFilter {
    ...
    private UsernamePasswordAuthenticationToken authentication(PortalIdentity identity) {
        String role = identity.roles().contains("ADMIN") ? "ADMIN"
                : identity.roles().contains("DATA_OPERATOR") ? "OPERATOR" : "RESEARCHER";
        return new UsernamePasswordAuthenticationToken(identity.username(), null,
