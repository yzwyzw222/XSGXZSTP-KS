...
import com.xsyu.academicgraph.application.auth.Permissions;
...
public final class PortalAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (path.matches("/api/v1/admin/users(?:/.*)?")) {
            reject(response, 410, "请通过统一门户管理平台账号。");
            return;
        }
    ...
    private UsernamePasswordAuthenticationToken authentication(PortalIdentity identity) {
        String role = identity.roles().contains("ADMIN") ? "ADMIN"
                : identity.roles().contains("DATA_OPERATOR") ? "ANALYST" : "RESEARCHER";
        Set<String> permissions = role.equals("RESEARCHER") ? Set.of("data:read", "analytics:read")
                : Permissions.PERMISSIONS.get(role);
        var authorities = new java.util.ArrayList<SimpleGrantedAuthority>();
        authorities.add(new SimpleGrantedAuthority("ROLE_" + role));
        permissions.forEach(permission -> authorities.add(new SimpleGrantedAuthority(permission)));
        var authentication = new UsernamePasswordAuthenticationToken(identity.username(), null, authorities);
        authentication.setDetails(new SessionUserResponse(identity.id(), identity.username(), identity.username(),
                List.of(role), permissions.stream().sorted().toList()));
