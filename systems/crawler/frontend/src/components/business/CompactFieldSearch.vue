...
<template>
  <div class="compact-search" role="search" aria-label="按字段搜索">
    ...
    <ElInput v-else class="compact-search__input" :model-value="modelValue" :aria-label="`${selected.label}搜索`" :placeholder="`搜索${selected.label}`" :maxlength="maxlength ?? 200" clearable @update:model-value="emit('update:modelValue', String($event ?? ''))" @keydown.enter="emit('submit')" />
...
.compact-search select { width: 78px; flex-shrink: 0; height: 36px; border: 1px solid hsl(var(--border)); border-radius: 4px; padding: 0 6px; background: hsl(var(--card)); color: hsl(var(--foreground)); font-size: 13px; }
