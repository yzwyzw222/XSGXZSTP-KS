...
<template>
  <section class="page-stack author-import">
    ...
    <section v-if="result" class="import-success" role="status"><CheckCircle2 :size="28" /><div><h2>导入已完成</h2><p>作者内部标识：{{ result.authorId }}</p><p>新增 {{ result.importedCount }} 项成果，补充关系 {{ result.linkedCount }} 项，已存在 {{ result.skippedCount }} 项。图谱正在后台同步，可稍后刷新查看。</p><RouterLink :to="authorGraphTarget(result.authorId)">查看 {{ result.scholarName }} 的知识图谱 →</RouterLink></div></section>
...
.import-steps { display: flex; list-style: none; gap: 28px; padding: 6px 12px 10px; color: hsl(var(--muted-foreground)); }
.import-steps li { display: flex; flex: 1; gap: 12px; align-items: center; font-size: 15px; }.import-steps li::after { content: ''; flex: 1; height: 2px; background: hsl(var(--border)); }.import-steps span { display: grid; place-items: center; width: 36px; height: 36px; flex-shrink: 0; border-radius: 50%; font-size: 16px; background: hsl(var(--secondary)); }.import-steps .active { color: hsl(var(--primary)); font-weight: 600; }.import-steps .active span { background: hsl(var(--primary)); color: hsl(var(--primary-foreground)); }
...
.import-file { display: grid; justify-items: center; align-content: center; min-height: 250px; gap: 14px; padding: 26px; text-align: center; border: 1px dashed hsl(var(--primary) / .5); border-radius: 6px; background: hsl(var(--primary) / .035); overflow-wrap: anywhere; }.import-file svg { color: hsl(var(--primary)); }.import-file p { font-size: 12px; color: hsl(var(--muted-foreground)); }.import-file input { max-width: 100%; font-size: 13px; }.import-file input::file-selector-button { padding: 10px 20px; margin-right: 12px; color: hsl(var(--primary-foreground)); background: hsl(var(--primary)); border: 1px solid hsl(var(--border)); border-radius: 4px; cursor: pointer; }
.import-guide { padding: 24px 28px; border: 1px solid hsl(var(--border)); border-radius: 10px; background: hsl(var(--accent) / .35); align-self: stretch; }.import-guide__eyebrow { letter-spacing: .12em; font-size: 11px; color: hsl(var(--primary)); }.import-guide h2 { margin: 16px 0; font-size: 25px; line-height: 1.5; font-weight: 650; }.import-guide p { font-size: 13px; line-height: 1.9; color: hsl(var(--muted-foreground)); }.import-guide ul { list-style: none; padding: 0; margin: 26px 0; display: grid; gap: 17px; }.import-guide li { display: flex; align-items: center; gap: 10px; font-size: 13px; }.import-guide svg { color: hsl(var(--primary)); }.import-guide__note { padding-top: 20px; border-top: 1px solid hsl(var(--border)); }
...
@media(max-width: 600px) { .import-steps { gap: 10px; }.import-steps li { font-size: 12px; gap: 5px; flex-direction: column; text-align: center; }.import-steps li::after { display: none; }.import-steps span { font-size: 14px; }.import-form { padding: 16px; }.import-mapping { grid-template-columns: 1fr; }.import-confirm { align-items: start; flex-direction: column; } }
</style>
