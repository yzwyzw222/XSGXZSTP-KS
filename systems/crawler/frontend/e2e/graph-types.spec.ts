...
test('图谱模块页签支持键盘访问并保持当前页高亮', async ({ page }) => {
  ...
  const nav = page.getByRole('navigation', { name: '模块导航', exact: true })
  await expect(nav.getByRole('link', { name: /学术.*图谱/ })).toHaveText(['学术关系图谱', '学术成果图谱', '学术背景图谱'])
  await expect(nav.getByRole('link', { name: '学术关系图谱', exact: true })).toHaveAttribute('aria-current', 'page')
  const entities = nav.getByRole('link', { name: '学术成果图谱' })
  await entities.focus()
  await page.keyboard.press('Enter')
  await expect(page).toHaveURL(/\/academic-achievements$/)
...
test('保存 MySQL 类型配置后 Canvas、图例和审核展示一致，合作关系可核对作品依据', async ({ page }) => {
  ...
  await page.goto('/graph/explore?centerType=AUTHOR&centerId=1')
...
test('实体与关系仅展示两类配置，支持详情及带版本的批量审核', async ({ page }) => {
  ...
  await page.goto('/graph/settings/edges')
  await expect(page.locator('.types-table tbody tr')).toHaveCount(2)
