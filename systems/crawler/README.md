...
**学术成果信息采集及可视化系统**，面向科研用户、数据运营人员和管理员，支持解析从知网导出的 XLSX、XLS、CSV 学者信息表，保存论文、专利、机构、硕博指导记录和摘要，并进行成果检索、统计与知识图谱浏览。

项目采用前后端分离架构：MySQL 保存业务数据，Neo4j 保存可重建的图投影，Vue 前端提供日常工作台、科研可视化大屏与模块工作区。本文以 **Windows 本地开发环境**为启动基线。

> 本目录由 **feature/Luo** 固定版本导入，当前集成代码在主仓库 `dev` 维护。集成启动请使用根目录 README；下方独立克隆步骤仅描述来源项目。

当前入口为 `/author-import`，详见 [作者导入说明](../../docs/author-import.md)。2026-09-11 已移除数据源、采集任务、数据治理和质量指标的页面及业务 API；历史采集数据、治理结果和基础表保留，旧计划在新版启动后停止执行。此前 [体验改进记录](../../docs/crawler-ux-improvements.md) 仅描述对应历史版本。

## 项目功能

全系统采用固定深蓝科研主题。登录后进入日常工作台，直接检索成果、查看最近导入的学者资料。原三栏可视化大屏保留在 `/dashboard`（集成入口为 `/crawler/dashboard`）；模块页使用横向导航，窄屏转为抽屉。菜单、数据请求和操作按账号权限开放。

| 模块 | 功能 |
| --- | --- |
| 工作台 | 检索成果，查看最近 6 次导入；从学者入口查看学术成果图谱。 |
| 可视化大屏 | 查看成果、作者、机构、有摘要成果数量、年度趋势、领域与类型分布、字段覆盖率、合作对、机构合作网络、最近作者导入及近期操作；支持手动刷新和区域独立降级。 |
| 成果目录 | 按题名、作者、机构、年份、类型、来源、载体和主题组合查询；选中候选后按规范实体 ID 精确匹配，自由输入保留模糊匹配；筛选和分页写入 URL，详情返回、浏览器后退与刷新可以恢复。快速预览成果，或进入完整详情查看署名、引用、知网原始列，以及已有历史来源与指标。 |
| 实体编目 | 查询作者、机构、期刊和主题；查看关联成果，以及可用的机构名称来源证据。 |
| 知识图谱 | 学术关系、学术成果、学术背景三个作者模块，展示作者、专利、论文、指导硕论与指导博论；支持共同创作证据、作者和作品详情、成果分类分页及日期时间线。原全局图谱、高级查询等保留在辅助工具中。 |
| 图谱设置 | 在节点样式与关系样式中维护固定类型的显示名称、颜色、尺寸及审核状态。 |
| 统计分析 | 查看发表趋势、字段覆盖率、成果类型、来源、机构、主题分布及作者/机构合作排行。 |
| 作者导入 | 上传同一学者的本人署名及导师筛选硕博文件，自动识别姓名、机构列表和关系；多人共同署名时点选候选。可分别调整工作表、表头和映射，所有文件校验后一起提交，保留原始字段。 |
| 数据导出 | 按当前已显示结果的筛选创建 CSV/JSON 异步导出，查询状态并下载结果；与目录共用后端筛选 SQL，不要求名称唯一。 |
...
- 每次导入一个工作表，文件最多 **10 MB、20 张工作表**，选中表最多 **2000 条、100 列**。管理员与数据运营人员可以导入；科研用户只读。导师身份由明确选择的硕博指导模式确定，不从第一责任人推断。
- 三类图谱以规范作者 ID 为中心，默认每页 20 项成果，支持分类与分页；详情只展示已收录资料。背景按成果日期排序，无日期置于末尾。具体口径见 [三类学术图谱](../../docs/academic-graphs.md)。
- 原全局概览在 `/graph/overview`；`/graph/settings/nodes`、`/graph/settings/edges` 是样式入口，`/graph/entities` 保留别名，`/graph/relations` 现为学术关系图谱。类型配置管理不等于直接新增、删除业务节点或关系。
- 每页图谱最多 **300 个节点**，先保留当前页作品，密集合作超限时显示提示；共同创作证据和合作作者数只覆盖当前页。合作须共同署名，导师指导不能替代署名。其他成果类型继续保留在目录。
...
| 信息表解析 | Apache POI 5.5.1、Apache Commons CSV 1.14.1 | XLSX/XLS 与 CSV 解析 |
| 数据访问 | MyBatis Spring Boot 4.1.0、MySQL、Flyway | 业务持久化、SQL 映射和版本化迁移 |
| 图数据 | Neo4j 5.26 Community、Spring Data Neo4j | 图投影、子图和路径查询 |
| 任务与会话 | Spring Batch、Quartz、Spring Session JDBC | 历史批处理兼容、图谱/运维调度和服务端会话 |
...
| 图表与图谱 | ECharts 6.1.0、Cytoscape.js 3.34.2 | 统计图表、统一图谱画布 |
| 校验与工具 | Element Plus 表单校验、Zod、VueUse、Lucide | 表单校验、图谱响应校验、响应式工具和图标 |
...
后端首次成功启动会执行 [Flyway 迁移](backend/src/main/resources/db/migration)（当前 V1–V16），创建业务表及 Session、Batch、Quartz 等基础表，并在启用引导且用户表为空时创建管理员。V16 新增每次采集运行的窗口快照表，保留已有任务、运行和计划。
...
空数据库中的目录、统计和图谱显示空态是正常的。进入“作者导入”，同时上传同一学者的本人署名成果表及可选的导师筛选硕博表，无需预填姓名、机构或关系。系统从完整署名交集识别学者，候选不唯一时点选；按来源库自动区分硕博指导，所有文件一起提交或回滚。仅有硕博文件时需附上本人署名表以识别导师。一次最多 10 份、合计 10 MB 和 2000 条；机构保留在成果上，不自动确认学者当前所属单位。`FirstDuty-第一责任人` 不用于自动确定学者或导师。MySQL 提交后，由现有 Outbox 异步更新图谱。
