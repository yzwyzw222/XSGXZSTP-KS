...
**学术成果信息采集及可视化系统**，面向科研用户、数据运营人员和管理员，支持从 OpenAlex、Crossref 采集学术成果，完成规范化入库、数据治理、成果检索、统计分析和知识图谱浏览。

项目采用前后端分离架构：MySQL 保存业务数据，Neo4j 保存可重建的图投影，Vue 前端提供科研可视化大屏与模块工作区。本文以 **Windows 本地开发环境**为启动基线。

> 当前完整实现位于 **feature/Luo** 分支。远端默认分支是 main，请按下方命令指定分支克隆。

## 项目功能

全系统采用固定深蓝科研主题。登录后进入三栏可视化大屏，通过中枢入口、数据面板和底部导航进入业务模块；模块页使用横向导航，窄屏转为抽屉。菜单、数据请求和操作按账号权限开放。

| 模块 | 功能 |
| --- | --- |
| 可视化大屏 | 查看成果、作者、机构、来源数量、年度趋势、领域与类型分布、字段覆盖率、合作对、机构合作网络、采集配置及近期操作；支持手动刷新和区域独立降级。 |
| 成果目录 | 按题名、作者、机构、年份、类型、来源、载体和主题组合查询；快速预览成果，或进入完整详情查看署名、引用、字段来源、人工修正和双源学术指标。 |
...
| 数据源与采集任务 | 按 OpenAlex、Crossref 名称选择来源；OpenAlex 作者、机构按名称搜索并多选，关键词限定成果主题；支持固定范围复查和持久化增量窗口、运行历史、每日计划、暂停、恢复、取消和检查点重试。 |
...
- 每次采集运行最多 **5 页、500 条**。原有计划仍复查固定范围；可显式选择增量模式，每次处理最多 7 天的窗口，完整成功后推进，超限时下次缩小窗口，失败时保留窗口。OpenAlex 按出版日期推进，Crossref 按索引更新时间推进；均不保证全库完整，具体边界见 [采集可靠性说明](../../docs/crawl-reliability.md)。
...
OpenAlex 任务中的作者、机构输入框按名称查询来源候选，结合所属机构或所在地确认后可多选；已有任务自动回显名称。内部继续保留来源标识，名称查询失败不会删除原条件。关键词可留空：填写时在 OpenAlex 标题、摘要及可检索全文中搜索，Crossref 则使用通用成果元数据查询，并与其他条件共同限定范围。例如 `graph neural networks` 用来筛选这一主题；搜索文字作为一个完整查询传给来源，不自动把逗号拆成多个主题。名称查询依赖外部服务，失败可在表单重试，机构可尝试英文名称或简称。
