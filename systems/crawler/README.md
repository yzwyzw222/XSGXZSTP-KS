...
| 模块 | 功能 |
| --- | --- |
| 工作台 | 检索成果，查看最近 6 次导入；从学者入口查看两跳知识图谱。 |
...
- 图谱概览的关键词、节点类型和关系筛选只处理已加载数据；刷新与切换中心才发起新的读取。`/graph/settings/nodes` 与 `/graph/settings/edges` 为样式入口，旧 `/graph/entities`、`/graph/relations` 地址继续可访问。
