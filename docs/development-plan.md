...
- 形成固定OpenAlex官方地址的数据源配置与探测、有界一次性和每日滚动发表日期窗口任务、Quartz JDBC计划、Spring Batch Chunk、业务检查点、暂停/恢复/取消、异常重启协调和有限失败重试闭环；
...
7. OpenAlex免费或基础访问下的每日模式定义为“滚动发表日期窗口幂等刷新”，不能宣称完整捕获历史记录更新。只有确认账号具备 `from_updated_date` 同步权限后，才可另行启用完整更新时间增量模式。
...
- 可以创建有界一次性任务和每日计划；未具备付费同步权限时，API和文档明确其为滚动发表日期窗口刷新，而非完整变更同步；
