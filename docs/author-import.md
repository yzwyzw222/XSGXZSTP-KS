...
适用版本：2026-09-12 的 crawler 系统。目录和集成地址继续使用 `systems/crawler`、`/crawler/`，当前仅保留这一业务系统。作者导入页已移除 ORCID 补全操作，只显示作者内部标识；后台获取也已移除，已有编号的保留方式见 [ORCID 说明](author-orcid.md)。下文 2026-09-11 的运行和验收数据为对应历史阶段。
