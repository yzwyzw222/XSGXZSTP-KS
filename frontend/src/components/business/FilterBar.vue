...
<template>
  <section :class="cn('filter-bar', props.class)" aria-label="筛选条件">
    <div v-if="collapsible" class="mb-3 flex items-center justify-between">
      ...
      <ElButton text size="small" :aria-expanded="!collapsed" @click="emit('toggle')">
        {{ collapsed ? '展开' : '收起' }}
        <ChevronDown class="size-4 transition-transform" :class="collapsed ? '' : 'rotate-180'" aria-hidden="true" />
      </ElButton>
    </div>
