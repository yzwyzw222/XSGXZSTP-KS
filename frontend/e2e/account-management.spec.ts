...
test('新建资料与统一编辑保存，并保留用户名只读和管理员自我保护', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '用户类型分布' })).toHaveCount(0)
...
test('从最近登录进入全部日志并分页筛选及查看来源详情', async ({ page }) => {
  ...
  await page.goto('/users/overview')
  ...
  await page.getByRole('combobox', { name: '结果', exact: true }).click()
...
test('统计失败不影响列表和登录日志', async ({ page }) => {
  ...
  await page.locator('a[href="/users/overview"]').click()
  await expect(page).toHaveURL(/\/users\/overview$/)
  await expect(page.getByText('统计暂不可用')).toBeVisible()
  await expect(page.getByRole('heading', { name: '系统账号' })).toHaveCount(0)
...
test('账号子模块切换保留未完成的保存并同步账号概况', async ({ page }) => {
  const state = await mockManagement(page)
  state.holdCreation()
  await page.goto('/users')
  await expect(page.getByRole('cell', { name: '林研究员', exact: true })).toBeVisible()
  await page.getByRole('button', { name: '新增用户' }).click()
  await page.getByLabel('用户名', { exact: true }).fill('section-researcher')
  await page.getByLabel('初始密码').fill('synthetic-test-password')
  await page.locator('#create-realName').fill('子模块切换测试')
  await page.getByRole('button', { name: '创建用户', exact: true }).click()
  ...
  await expect.poll(() => state.requests.filter((request) => request.path === '/api/v1/users/statistics').length).toBe(2)
  await page.getByRole('navigation', { name: '账号管理内容' }).getByRole('link', { name: '系统账号', exact: true }).click()
  await expect(page.getByRole('cell', { name: '子模块切换测试', exact: true })).toBeVisible()
})
...
test('离开账号页面后忽略迟到的保存响应，不再刷新统计或显示成功通知', async ({ page }) => {
  ...
  await expect.poll(() => state.requests.filter((request) => request.path === '/api/v1/users' && request.method === 'POST').length).toBe(1)
  await page.keyboard.press('Escape')
  const cancelled = page.waitForEvent('requestfailed', (request) => request.method() === 'POST' && new URL(request.url()).pathname === '/api/v1/users')
