...
const destinations = {
  crawler: ['catalog/master-theses', 'graph/overview'],
}
...
try {
  ...
  for (const system of config.systems) {
    ...
    await page.waitForURL(url => url.pathname === '/crawler/')
    for (const destination of destinations[system.id]) {
      ...
      await page.locator('.research-brand').waitFor()
      ...
      if (system.id === 'crawler' && destination === 'graph/overview') await verifyCrawlerGraph(page)
    }
    await page.screenshot({ path: fileURLToPath(new URL(`${system.id}-desktop.png`, screenshots)), fullPage: true, animations: 'disabled' })
    if (development) assert.ok(hmrConnected, `${system.id} HMR 必须通过系统网关建立连接`)
    await page.goto(base + '/?workspace=' + encodeURIComponent('/crawler/graph/overview'))
    await verifyCrawlerGraph(page)
    await page.getByRole('button', { name: '刷新图谱', exact: true }).click()
    await page.getByRole('button', { name: '适应画布', exact: true }).click()
    await verifyCrawlerGraph(page)
    assert.equal(await page.locator('iframe').count(), 0, '成果系统直接提供业务页面')
    assert.deepEqual(errors, [], `${system.id} 页面脚本或后端响应异常`)
    await page.close()
    process.stdout.write(`${system.id}：统一登录后直接访问、两个业务页面、深链接刷新、会话保持通过。\n`)
    if (system.id === 'crawler') process.stdout.write('crawler：业务页与兼容书签的图谱实际绘制、刷新和适应画布检查通过。\n')
  }
} finally {
  try { await logoutPortal(context.request, base) }
  catch { process.stderr.write('统一验收会话清理失败，请检查网关状态。\n') }
  await context.close()
  await browser.close()
}
