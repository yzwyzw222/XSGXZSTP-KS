...
$plan = Get-IntegrationPlan
Assert-Condition ($plan.processes.Count -eq 0) '该验收用于三个系统全部维护中的第一阶段。'
...
try {
    & (Join-Path $PSScriptRoot 'Start-Integration.ps1') -System all -Mode Demo
    $records = @(Read-IntegrationState)
    Assert-Condition ($records.Count -eq 1 -and $records[0].system -eq 'portal') '维护模式启动了非门户进程。'
    ...
    Assert-Condition (Test-IntegrationIdentity $portalRecord) '停止维护中系统影响了门户。'
    ...
    & (Join-Path $PSScriptRoot 'Stop-Integration.ps1') -System all
    & (Join-Path $PSScriptRoot 'Start-Integration.ps1') -System extraction -Mode Development
    Assert-Condition (@(Read-IntegrationState).Count -eq 1) '显式选择维护系统时启动了后端。'
    & (Join-Path $PSScriptRoot 'Stop-Integration.ps1') -System all
